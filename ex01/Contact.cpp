...
/*   Updated: 2022/03/09 20:00:58 by cjeon            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool Contact::is_valid_phone_number(\
  ...
  if (phone_number.length() < Contact::min_phone_number_len_ \
    ...
    return false;
  return is_valid_format(phone_number, \
                         Contact::is_valid_phone_number_char);
}
...
{
  bool is_valid = is_valid_name(nickname);
  if (is_valid)
    nickname_ = nickname;
  return is_valid;
}
...
std::ostream &operator<<(std::ostream &ostream, Contact contact) {
  ostream << std::setw(10) << contact.firstname() << '|';
  ostream << std::setw(10) << contact.lastname() << '|';
  ostream << std::setw(10) << contact.nickname();
  return ostream;
}